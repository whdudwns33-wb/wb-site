...
| **프로그램 방** ×6 | 그 프로그램의 오늘 카드 · 공식 사이트 열기 · 매뉴얼 · 현황(구독 학생별 마지막 카드) · (원장) 반복 템플릿. 이그잼포유·족보닷컴 방에는 **앱 자료 범위**(앱별 과목·학년·단원·자료 종류 — 없음/구매 대기/업로드 대기/있음, 직원이 진행 상태를 옮긴다) |
...
├─ migrations/         D1 스키마 (멱등) — 001 기본 표, 002 문서 저장소 desk_documents(컬렉션 CHECK 없음, 허용 목록은 desk-api.mjs), 003 파일(desk_files, 매뉴얼 사진 BLOB)
