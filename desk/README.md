...
| **매뉴얼** | 프로그램 × 작업 종류(배정·확인·다운로드·앱 업로드·계정·후속 연락)별 목적·단계·주의·공식 링크·마지막 확인일. 원장이 편집, [기본 매뉴얼 불러오기]로 초안 16개. 직원은 다른 점을 요청함(기타)으로 |
...
| **매뉴얼** | `manuals` — 방(또는 app)·작업 종류·제목·목적·단계·주의·링크(https만)·마지막 확인일·버전 | 원장 |
...
├─ migrations/         D1 스키마 (멱등) — 001 기본 표, 002 문서 저장소 desk_documents(컬렉션 CHECK 없음, 허용 목록은 desk-api.mjs)
