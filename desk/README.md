...
(id 가 `p:<템플릿>:<날짜>:<대상>`으로 결정적이라 어느 기기가 몇 번 만들어도 하나) ② 원장 [＋ 지시] ③ (예정) 요청함 `assignment`.
원장이 지운 카드는 되살아나지 않는다(서버가 아는 id 는 건너뛴다).
...
| **반복 템플릿·앱 자료 범위** | `plans` — `recurring`(프로그램·요일·대상·무엇·어디에·기간·켜짐) / `apprange`(앱·과목·학년·범위·자료 종류·출처·상태) | 원장. 직원은 `apprange`의 상태·메모만(서버가 그 외 변경을 403) |
