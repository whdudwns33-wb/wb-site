...
t('흰 글씨라고 다 정답은 아니다 — 연 라벨·마스트헤드·배너를 거른다', () => {
  ...
  assert.ok(decoy({ text: '2', size: 11, x: 113, color: '#ffffff' }, 69.4, '중 2 국어 1-1.저녁 항구'));
  /* 머리말·판권 띠 */
...
t('연 나눔의 큰 간격은 대개 끼어든 날개풀이다 — 그 높이를 빼야 진짜 연이 보인다', () => {
  /* 실측 이해완성 A의 y값을 그대로 옮긴 것. 순진한 간격 추정은 여기서 6연을 5연으로 읽었다. */
  ...
  /* 이해완성 B(비린내라뇨!)의 실측 y — 행 수가 연마다 다른 시다 */
...
t('절 경계는 마스트헤드 한 신호로 잡는다', () => {
  const doc = { pages: [
    pg(1, [ln(69.4, [sp(102, '중2 국어 1-1.(1)저녁 항구')])]),
    pg(2, [ln(42, [sp(60, '1-1.(1)저녁 항구')])]),
    pg(3, [ln(69.4, [sp(102, '중2 국어 1-1.(2)비린내라뇨!')])]),
  ] };
...
t('머리말 메타는 마스트헤드와 단원 코드가 한 줄에 와도 둘 다 읽는다', () => {
  ...
  /* 작품 코드 '1-1.(1)저녁 항구' 는 작품명으로 읽힌다 */
  const w = U.headerMeta({ pages: [pg(1, [ln(69.4, [sp(102, '중2 국어 1-1.(1)저녁 항구')])])] });
  assert.strictEqual(w.workTitle, '저녁 항구');
