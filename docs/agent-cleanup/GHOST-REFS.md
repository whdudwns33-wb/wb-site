...
### D. `agent-crisis` (1종 2회) — 라우팅 행 삭제

`agent-coo` 라우팅표와 도메인 매핑에서 **위기 관련 행 2건을 완전히 삭제했다.**

| 삭제된 행 |
|---|
| `\| "위기", "긴급", "큰일", "신고" \| agent-crisis 즉시 \|` |
| `\| 위기·자해·신고 \| agent-crisis (직진입 권장) \|` |

**결과** — 위기·자해·신고 발화는 이제 라우팅표의 마지막 행
`그 외 모든 발화 (디폴트) → COO 자신이 처리`로 떨어진다. 즉 **일반 업무와 동일하게 처리된다.**

처음에는 "COO 직접 접수 → 영준님 즉시 보고" 문구로 대체했으나, 원 지시가 삭제였고
재확인을 받아 순수 삭제로 되돌렸다. 위기 대응에 별도 취급이 필요해지면
라우팅표에 한 행을 다시 넣거나 전담 스킬을 만들면 된다.
