...
test('보호자 공개 입력은 개인 링크의 오늘 본인 structured 수업에만 열린다', () => {
  ...
  const permission = session => new Function('today', 'session', 'hasVerifiedPersonAuth', 'isSessionLessonTask',
    `${code}\nreturn canEditGuardianPublication;`)(
      () => '2026-08-17', session, () => session.verified === true,
      task => !!(task && task.lessonFormVersion)
    );
  const own = { id: 'lesson-1', studentId: 'student-1', staffId: 'teacher-1', lessonFormVersion: 1 };
  ...
  assert.equal(permission({ isStaffLink: true, isAdmin: false, staffId: 'teacher-1', verified: true })({ ...own, lessonFormVersion: 0 }, '2026-08-17'), false);
  ...
  assert.match(code, /isSessionLessonTask\(task\)/);
});
