/**
 * 기존 교재 수정 신청 — textbooks.json 정적 항목이든, book-add-request로 승인 등록된
 * 항목이든 마찬가지로 앱에서 직접 못 고친다. 선생님이 "이렇게 고쳐 주세요"를
 * 신청하면(request), 원장이 검토(review)해 승인해야 실제 목록에 반영된다.
 * 원장이 직접 신청하면 검토 없이 바로 승인 상태로 등록된다.
 *
 * book-add-request.js와 거의 같은 골격이지만 키가 다르다 — 추가는 "교재명"으로
 * 중복을 막고(같은 책을 두 번 만들지 않도록), 수정은 "대상 book_id"로 하나만
 * 열려 있게 막는다(같은 책에 대한 제안이 여러 개 동시에 떠 있지 않도록).
 * 수정 신청은 그때그때의 필드 값 전체를 스냅샷으로 받는다(부분 diff가 아니다) —
 * 클라이언트가 수정 모달을 열 때 현재 값을 채워 보여주고, 그 값을 그대로 또는
 * 고쳐서 제출하는 방식이라 "안 건드린 값 = 원래 값 유지"가 자연스럽게 된다.
 *
 *   POST /book-edit-request { app, auth, action:'submit'|'cancel'|'list'|'list_approved', bookId, title, subject, level, vendorName, units, note }
 *   POST /book-edit-review  { app, auth(admin), action:'list'|'approve'|'reject', requestKey, revision, note }
 */
...
function contentPayload(fields) {
  return { title: fields.title, subject: fields.subject, level: fields.level, vendorName: fields.vendorName, units: fields.units, note: fields.note };
}

/** 신청자가 아니라 "대상 책"으로만 키를 만든다 — 같은 책에 대한 제안은 하나만 열려 있다 */
async function requestKeyFor(bookId) {
  return 'ber_' + (await sha256Hex(bookId)).slice(0, 48);
}
...
export async function handleBookEditRequest(env, app, body, origin, auth, json) {
  ...
  if (action === 'list_approved') {
    ...
    const result = await env.DB.prepare(
      "SELECT * FROM book_edit_requests WHERE app=? AND status='approved' ORDER BY updated_at DESC LIMIT " + limit
    ).bind(app).all();
    return json({ ok: true, edits: (result.results || []).map(approvedView) }, 200, origin);
  }
  ...
  if (action === 'list') {
    ...
    const result = await env.DB.prepare(
      "SELECT * FROM book_edit_requests WHERE app=? AND owner=? ORDER BY CASE status " +
  ...
  if (!SAFE_ID.test(bookId)) return json({ ok: false, error: '올바른 bookId가 필요합니다' }, 400, origin);
  const requestKey = await requestKeyFor(bookId);
  const now = Date.now();
  let current = await env.DB.prepare('SELECT * FROM book_edit_requests WHERE app=? AND request_key=? LIMIT 1')
  ...
  if (action === 'cancel') {
    ...
    const result = await env.DB.prepare(
      "UPDATE book_edit_requests SET status='cancelled', revision=revision+1, updated_at=?, " +
    ...
    current = await env.DB.prepare('SELECT * FROM book_edit_requests WHERE app=? AND request_key=? LIMIT 1')
      .bind(app, requestKey).first();
  ...
  const fields = { title, subject, level, vendorName, units, note };
  ...
  if (!current) {
    await env.DB.prepare(
      'INSERT OR IGNORE INTO book_edit_requests ' +
  ...
  if (current.content_hash === contentHash && current.owner === owner && current.status !== 'cancelled' && current.status !== 'rejected') {
    return json({ ok: true, idempotent: true, request: view(current) }, 200, origin);
  }

  const result = await env.DB.prepare(
    'UPDATE book_edit_requests SET owner=?, title=?, subject=?, level=?, vendor_name=?, units=?, note=?, content_hash=?, ' +
    "revision=revision+1, status=?, updated_at=?, reviewed_at=?, reviewed_by=?, review_note=NULL " +
    'WHERE app=? AND request_key=? AND revision=?'
  ).bind(owner, title, subject, level, vendorName, unitsJson, note, contentHash, status, now, reviewedAt, reviewedBy, app, requestKey, Number(current.revision)).run();
  if (Number(result && result.meta && result.meta.changes || 0) !== 1) {
    return json({ ok: false, error: '다른 변경이 먼저 저장되었습니다. 새로고침 후 다시 시도해 주세요' }, 409, origin);
  }
  current = await env.DB.prepare('SELECT * FROM book_edit_requests WHERE app=? AND request_key=? LIMIT 1')
...
export async function handleBookEditReview(env, app, body, origin, auth, json) {
  if (app !== 'task') return json({ ok: false, error: '이 기능은 직원 앱에서만 사용할 수 있습니다' }, 400, origin);
  ...
  if (action === 'list') {
    ...
    const limit = Math.max(1, Math.min(200, Number(body.limit) || 100));
    const result = await env.DB.prepare(
      'SELECT * FROM book_edit_requests WHERE ' + clauses.join(' AND ') +
  ...
  if (!SAFE_ID.test(requestKey) || !requestKey.startsWith('ber_')) {
