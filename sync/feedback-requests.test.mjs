...
function seedTask(db, id, owner, overrides = {}) {
  const now = Date.now();
  const data = { id, staffId: owner, title: '[정규] 진호윤(중2) — 국어 독해', studentName: '진호윤', deleted: false, ...overrides };
...
test('submission resolves teacher name from the staff table, not from client input', async () => {
  ...
  assert.equal(result.body.request.studentName, '진호윤', '지시서 제목/필드에서 학생 이름을 뽑는다');
});
