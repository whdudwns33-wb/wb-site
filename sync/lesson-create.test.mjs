...
test('all nine form groups are enforced and literal 없음 is accepted', async () => {
...
test('person auth cannot target another teacher', async () => {
  const db = new FakeDB();
  const { response } = await call(db, { staffId: 'teacher-2', lesson: validLesson() }, { scope: 'own', id: 'teacher-1' });
...
test('client cannot inject server-owned task fields', async () => {
  const db = new FakeDB();
  const lesson = validLesson({ origin: 'manager' });
...
test('changed grade, schedule, and start update the same task with a revision', async () => {
  const db = new FakeDB();
  const first = await call(db, { lesson: validLesson() }, { scope: 'own', id: 'teacher-1' });
  const correctedLesson = validLesson({
    grade: '초5',
    start: '2026-09-01',
    scheduleText: '금 18:00-19:00',
    scheduleSlots: [{ days: [5], startTime: '18:00', endTime: '19:00' }]
...
test('a changed submission without an expected version cannot overwrite', async () => {
  ...
  const unsafe = await call(db, {
    lesson: validLesson({ materials: '기준 없이 덮어쓰기' })
  }, { scope: 'own', id: 'teacher-1' });
...
test('stale expectedUpdatedAt returns an optimistic conflict without overwrite', async () => {
  ...
  const second = await call(db, {
    lesson: validLesson({ materials: '수정 교재 1' }),
    expectedUpdatedAt: first.data.task.updatedAt
  }, { scope: 'own', id: 'teacher-1' });
  const stale = await call(db, {
    lesson: validLesson({ materials: '수정 교재 2' }),
    expectedUpdatedAt: first.data.task.updatedAt
  }, { scope: 'own', id: 'teacher-1' });
...
test('staff cannot overwrite a manager-created assignment', async () => {
  const db = new FakeDB();
  const managed = await buildLessonTask(validLesson(), 'teacher-1', 'manager', 100);
  seed(db, managed);
  const result = await call(db, {
    lesson: validLesson({ materials: '직원 임의 수정' }),
...
test('a deleted matching assignment returns 409 instead of a hidden success', async () => {
  const db = new FakeDB();
  const deleted = await buildLessonTask(validLesson(), 'teacher-1', 'staff', 100);
  deleted.deleted = true;
  seed(db, deleted);
  const result = await call(db, { lesson: validLesson() }, { scope: 'own', id: 'teacher-1' });
...
test('wrong-owner deterministic id is neither returned nor modified', async () => {
  const db = new FakeDB();
  const candidate = await buildLessonTask(validLesson(), 'teacher-1', 'staff', 100);
  const foreign = {
    ...candidate,
    staffId: 'teacher-2',
    origin: 'staff',
...
test('sourceTaskId is owner scoped', async () => {
  const db = new FakeDB();
  const foreign = await buildLessonTask(validLesson(), 'teacher-2', 'staff', 100);
  seed(db, foreign);
  const result = await call(db, {
