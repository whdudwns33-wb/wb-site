...
test('director can register a phone and toggle consent', async () => {
  const db = new TestD1(); seedStaff(db);
  const set = await call(db, { auth: admin, action: 'set', studentName: '진호윤', phone: '010-1234-5678', consent: true });
  ...
  assert.equal(list.body.contacts[0].studentName, '진호윤');
});

test('invalid phone format is rejected', async () => {
  const db = new TestD1(); seedStaff(db);
  const bad = await call(db, { auth: admin, action: 'set', studentName: '진호윤', phone: '123', consent: false });
  assert.equal(bad.status, 400);
});

test('consent cannot be turned on without a phone number', async () => {
  const db = new TestD1(); seedStaff(db);
  const r = await call(db, { auth: admin, action: 'set', studentName: '진호윤', phone: '', consent: true });
  assert.equal(r.status, 400);
});

test('re-saving updates the same row (upsert), not a duplicate', async () => {
  const db = new TestD1(); seedStaff(db);
  await call(db, { auth: admin, action: 'set', studentName: '진호윤', phone: '01011112222', consent: true });
  await call(db, { auth: admin, action: 'set', studentName: '진호윤', phone: '01033334444', consent: false });
...
test('empty phone clears the contact (consent forced off)', async () => {
  const db = new TestD1(); seedStaff(db);
  await call(db, { auth: admin, action: 'set', studentName: '진호윤', phone: '01011112222', consent: true });
  const cleared = await call(db, { auth: admin, action: 'set', studentName: '진호윤', phone: '', consent: false });
