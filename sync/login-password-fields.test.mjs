...
for (const app of ['consult', 'task']) {
  ...
  const versionPath = app === 'consult'
    ? path.join(root, 'consult', 'version.json')
    : path.join(root, 'version.json');
  const version = JSON.parse(fs.readFileSync(versionPath, 'utf8')).v;
  ...
  test(`${app} static version matches its version.json`, () => {
