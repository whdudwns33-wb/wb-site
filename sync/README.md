...
- `TASK_MANAGER_STAFF_IDS_CONFIG`는 관리 권한, `TASK_WORK_LOGIN_MANAGER_IDS_CONFIG`는
  그중 일일 출퇴근 로그인을 적용할 직원 ID 목록이다. 이름과 표시 순서는 바꾸지 않는다.
- 근무 로그인 대상 관리자는 private profile의 연락처와 해시 PIN을 설정하고
  `login_enabled=1`로 사용한다. 개인 기기 인증만으로 업무 데이터나 직원 연락처에 접근할 수 없고,
  출근 로그인 후에는 다른 관리자와 같은 기능을 사용한다.
- 기존 로그인 면제 관리자와 원장 인증은 유지한다. 새 근무 관리자도 최초 로그인·최종 로그아웃을
  출퇴근 요약으로 기록하며, 기기 연결은 로그아웃으로 해제하지 않는다.
...
> `077`은 실제 주말 등·하원 기록의 열린 상태를 학생·방문일별로 제한한다. 이전 날짜의
> 하원 누락은 오늘 등원을 막지 않고 관리자 보정 목록에 날짜와 함께 표시하며, 같은 날짜의
> 열린 중복 기록은 계속 차단한다.
