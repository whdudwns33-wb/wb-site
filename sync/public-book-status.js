...
import { INTERNAL_BOOK_DELIVERY } from './book-order-create.js';

const SAFE_ID = /^[A-Za-z0-9_-]{1,128}$/;
const IMMEDIATE_RECEIPT_DELIVERIES = new Set(['bound_print_v1', INTERNAL_BOOK_DELIVERY]);
const ORDER_DELIVERIES = new Set([
  'scheduled_batch_v1', 'manual_online_v1', ...IMMEDIATE_RECEIPT_DELIVERIES
]);
const ISSUE_STAGES = {
...
export async function readPublicBookStatus(env, studentId, now = Date.now()) {
  ...
  for (const [taskId, taskRows] of snapshotByTask) {
    ...
    for (const [itemIndex, itemRows] of byItem) {
      ...
      const immediateReceipt = IMMEDIATE_RECEIPT_DELIVERIES.has(delivery);
      if (immediateReceipt && !fulfillment) continue;
      if (fulfillment && !immediateReceipt && (!send || String(send.status) !== 'accepted')) continue;
