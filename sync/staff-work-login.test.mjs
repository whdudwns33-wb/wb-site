...
test('login records server clock, binds opaque session to durable device, and logout preserves first punches', async () => {
  ...
  await withNow(now, async () => {
    ...
    const second = await withNow(now + 4000000, () => call(env, 'login', { pin }));
    assert.equal(second.status, 200);
    assert.equal(second.data.attendance.record.at, now);
    assert.equal(second.data.attendance.record.out, null);
    assert.equal(second.data.attendance.record.sessions.length, 2);
    const secondRequest = { auth: { ...body('logout').auth, workSession: second.data.workSession } };
    const secondOut = await withNow(now + 5000000, () => call(env, 'logout', secondRequest));
    assert.equal(secondOut.status, 200);
    assert.equal(secondOut.data.attendance.record.out, now + 5000000);
    assert.equal(secondOut.data.attendance.record.sessions.length, 2);
    const again = await withNow(now + 6000000, () => call(env, 'logout', secondRequest));
    assert.equal(again.data.attendance.record.out, now + 5000000);
    assert.equal((await gate(env, first.data.workSession)).data.code, 'STAFF_WORK_LOGIN_REQUIRED');
    assert.equal((await gate(env, second.data.workSession)).data.code, 'STAFF_WORK_LOGIN_REQUIRED');
    const third = await withNow(now + 7000000, () => call(env, 'login', { pin }));
    assert.equal(third.status, 200);
    assert.equal(third.data.attendance.record.sessions.length, 3);
