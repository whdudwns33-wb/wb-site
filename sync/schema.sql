...
-- 아카등록 완료된 일반 주문 교재의 공용 카탈로그. 학생·주문 식별자는 저장하지 않는다.
...
-- 선생님→관리자 실시간 요청. 지정 수신자는 책임 표시용이며 모든 관리자가 같은 요청을 조회한다.
CREATE TABLE IF NOT EXISTS teacher_live_requests (
  ...
  lesson_task_id      TEXT    NOT NULL CHECK (length(lesson_task_id) BETWEEN 1 AND 128),
  lesson_date         TEXT    NOT NULL CHECK (lesson_date GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]'),
  student_id          TEXT    NOT NULL CHECK (length(student_id) BETWEEN 1 AND 128),
...
-- 학생 단위 4회제의 3회 출석 확정 알림. 이름·연락처는 저장하지 않고 roster에서 표시한다.
CREATE TABLE IF NOT EXISTS tuition_generation_alerts (
  app               TEXT    NOT NULL CHECK (app = 'task'),
  alert_id          TEXT    NOT NULL CHECK (
    length(alert_id) = 56 AND alert_id GLOB 'tga_[0-9a-f]*'
    AND substr(alert_id,5) NOT GLOB '*[^0-9a-f]*'
  ),
  student_id        TEXT    NOT NULL CHECK (length(student_id) BETWEEN 1 AND 128),
  cycle_start_date  TEXT    NOT NULL CHECK (cycle_start_date GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]'),
...
CREATE TABLE IF NOT EXISTS tuition_generation_alert_confirmations (
  ...
  confirmation_id TEXT    NOT NULL CHECK (
    length(confirmation_id) = 56 AND confirmation_id GLOB 'tgc_[0-9a-f]*'
...
-- consult 문화상품권 교환이 처리 중인 학생은 삭제하거나 대표·관리자로 전환할 수 없다.
-- Worker 선조회와 함께 D1에서 claim/직원 변경 경합을 원자적으로 차단한다.
