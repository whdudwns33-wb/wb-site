...
이후 아래 후속 배포에서 적용됐으므로 운영 DB에 다시 실행하지 않는다.

## 2026-08-18 KST 학생 자기 체크 후속 배포 결과

배포 전 probe에서 040이 정확히 `0/15`, `0/4`인 것을 확인한 뒤 한 번 적용했고, 사후 probe에서
`15/15`, `4/4`를 확인했다. 보호자·직원 Worker와 학생 Worker health는 모두 HTTP `200`, 공개 Origin
경계는 각각 `401/403`, 원격 task·consult·version은 `2026-08-18.2`로 확인했다. 릴리스 커밋은
`1419229`이다. GitHub Pages는 GitHub API 장애로 처음 두 요청이 HTTP `500/503`을 반환했지만,
동일 커밋 재요청의 build와 deploy가 모두 성공했고 실제 원격 버전까지 다시 확인했다.

040은 운영에 적용됐으므로 재실행하지 않는다. 이후 변경은 새 migration으로만 추가한다.
...
학생 앱 배포가 기존 보호자 앱을 바꾸지 않았는지 같은 시험 학생으로 확인한다.

1. 기존 v2 보호자 기기에서는 오늘 수업·출결·차량만 추가로 보이고, v3 숙제·요청과 v4 공지·교재가
   보이지 않는지 확인한다. 기존 v1~v3 연결은 원래 공개 범위를 그대로 유지해야 한다.
2. 관리자에서 보호자 화면 미리보기를 열기 전후 access/code/session 행 수가 바뀌지 않았는지
   비식별 개수만 확인한다.
3. 보호자 공개 v4 범위를 다시 설명하고 동의를 저장한다. 기존 v2 세션을 새로고침하면 접근이
   거절되어야 한다.
4. 새 초대 링크를 보호자 휴대폰에서 열어 fragment가 즉시 사라지는지 확인한다. 오늘 현황, 공개
   숙제·준비물, 학원 공지, 교재 상태, 시간표, 보강·회차·피드백, 보호자 요청이 관리자 미리보기와
   같은 순서와 내용으로 보이는지 확인한다.
5. 시험용 `상담 요청` 한 건만 접수하고 관리자 통합 요청함에서 처리 완료한다. 자유문자·연락처가
   요청 payload나 보호자 화면에 생기면 실패다. 실제 보강 건이 없으면 보강 응답을 만들지 않는다.
6. 별도 초대 링크로 태블릿도 연결해 같은 학생과 같은 공개 범위만 보이는지 확인한다.
7. 관리자에서 보호자 앱 연결을 끈 뒤 휴대폰·태블릿을 새로고침해 모두 접근이 거절되는지 확인한다.
8. 보호자 세션 쿠키와 학생 세션 쿠키가 서로의 Worker API에서 인정되지 않는지 확인한다.
...
관리자 미리보기 무부작용: 통과/실패
기존 보호자 v2 범위 유지: 통과/실패
보호자 v4 휴대폰·태블릿 연결: 통과/실패
보호자 요청 접수·처리: 통과/실패
휴대폰 연결: 통과/실패
태블릿 연결: 통과/실패
허용 정보만 표시: 통과/실패
