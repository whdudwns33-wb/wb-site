...
| `hanja/` | 어휘브레인(옛 한자브레인 — **표시 이름만** 바뀌었다. 폴더·주소 `/hanja/`·KV 키 `hanja:*`·API 는 그대로) — 한글 어휘가 부족한 학생용 **한글 어휘·한자어** 앱(영어 없음). 문제집 한 권 = 단어장 하나(관리 웹 업로드, KV 전용, 낱말 id 는 내용 기반이라 재업로드에 안 밀린다), 종류는 자체(`own`)·교재(`textbook`) 둘이고 **AI 연상은 자체 단어장에서만**(교재 뜻 문장을 외부 AI 로 보내지 않는다), 한자는 캔버스에 손가락으로 따라쓰기 + 훈음→한자 쓰기 회상 문항(급수 교재처럼 뜻 없는 예시 낱말은 글자의 `words` 목록으로 받는다)(공용 획순 사전 `hanja:strokes` 가 있는 글자만 획순 판정). 이번 주 단원 지정(반·학생)·종이 시험지 인쇄·밤 9시 알림. **두 트랙** — 어휘(낱말이 있는 단어장)와 한자(글자만 있는 급수 단어장)를 탭에서 갈라 보여 준다(어휘 트랙은 한자 탭을 내리고 낱말 쪽에서 따라쓰기로 간다). **1차 목표는 「교재 점검」** — 센터에서 종이 교재로 공부하라고 보낸 단원을 앱에서 심지 않았어도 그대로 문제로 내고(답하면 심긴다), 단원마다 마지막 점수가 강사 현황·진도표로 올라간다. 순수 로직 `book-check.js`·`srs.js`·`quiz.js`·`trace.js`·`bridge.js`(진로독서 어휘장 다리) + 도구 `extract/draft.mjs`(교재 텍스트→초안)·`strokes-convert.mjs`(공개 획순→사전) + `book-sample.json`(자체 창작 체험 단어장) + `books/`(**자체 단어장** — 교과 어휘 초3~초6 · 각 80낱말 8단원 + 한자 급수 8급 50자·7급 100자(초안) — 학원이 쓴 자료라 저장소에 산다, `books.test.cjs` 가 업로드 관문과 같은 규칙으로 오류·경고 0 을 요구한다). **상세: `hanja/README.md`**, 고칠 때 깨지는 자리·인수인계: `hanja/AGENTS.md` |
...
- **어휘브레인 `/hanja/`**: 한글 어휘가 약한 학생이 **문제집 한 권(단어장)을 단원 순서대로** 익히는 앱. 영어 없음, 한자는 손으로 쓴다.
