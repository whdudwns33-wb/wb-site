...
원장이 구매 자료를 **드라이브에 올리면** 그 텍스트를 스튜디오에 붙여 팩을 만든다.
드라이브 폴더는 `WB 교재스캔` 하나를 쓰고, 그 아래를 **출판사/학년/과** 로 나눈다.
...
WB 교재스캔/
  ne-kimgitaek/          # 출판사(저자) — packId 의 가운데 조각과 같은 말
    m2/                  # 학년
      L6/                # 과
        01-내용정리.txt   # ← 대화문·청크 원천   (스튜디오 종류: dialogues, sentences)
        02-본문워크북.txt # ← 문장·핵심어·토큰   (sentences)
        03-워드테스트.txt # ← 단어              (words)
        07-그래머.txt     # ← 문법 패턴·문항     (patterns, items)
```

- **파일은 텍스트(.txt)** 로 올린다 — PDF 그대로는 못 읽는다(워커에 PDF 파서가 없다).
  PDF에서 전체 선택·복사해 메모장에 붙여 저장하면 된다.
- 파일 이름 앞의 번호는 이그잼포유 시리즈 번호를 그대로 쓴다 — 어느 자료가 어느 추출
  종류로 가는지가 이름만 보고 정해진다.
- **드라이브에 올린 원천도 저장소에는 절대 들어오지 않는다.** 스튜디오가 서버의 작업
  저장소(`naesin:jobsrc:*`)에만 담고, 작업을 지우면 원천도 함께 지운다.
...
**AI 비용 한도(2026-09-03, 기획서 §13-8 확정)** — 팩 제작 추출에 **하루 80회** 한도를 걸었다
(`NAESIN_AI_DAILY` 로 조절, 학생용 AI 한도와 장부를 따로 쓴다 — 팩 하나 만들다 학생들의 연상
만들기가 막히면 안 된다). 근거: 조각 하나가 대략 입력 5천·출력 5천 토큰이고 Opus 5 기준
$5/$25 per 1M 이라 호출 한 번이 약 **$0.15(200원 안팎)**, 과 하나가 종류별 조각을 합쳐
**10~15회 ≈ $2(3천원 아래)** 다. 80회면 하루 대여섯 과를 만들 수 있고, 잘못된 반복 호출이
있어도 그날 손실이 $12 선에서 멈춘다. 스튜디오 화면이 추출할 때마다 남은 횟수를 보여 준다.

한도를 넣으면서 드러난 결함도 함께 고쳤다 — 한도를 **추출 한 번마다** 세고 있었는데
