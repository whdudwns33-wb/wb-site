...
## 파일럿 운영 — 링크만으로 (관리 웹·PIN·학생 연동 없이)

`/letter/` 는 배포본에 실린 **호 목록**(`letter/issues.json`)을 읽고, 발행일이 지난 것 중 **가장 최근 호**를 링크만으로 연다(가족 링크·학생 토큰 없음).
아직 아무 호도 발행일이 안 됐으면 가장 이른 호를 미리 보여 준다(파일럿에 빈 화면은 없다). 지난 호는 [지난 호] 목록에 남아 언제든 다시 열린다.
학년대는 가족이 위 메뉴에서 고르고, 문제·미션·쓰기 기록은 기기에만 남는다(서버 기록·열람 현황·푸시는 없다).

**어느 링크로 들어와도 이번 주 호가 열린다.** 파일럿 기간에는 호가 서버(KV)가 아니라 배포본에 있어서, 가족 링크(`?t=`)로 들어오면
서버는 링크가 멀쩡해도 "호 없음" 이라고 답한다 — 그때 앱이 배포본 호로 넘어간다. 세 갈래가 이렇게 갈린다.

| 들어온 링크 | 보이는 것 | 기록이 남는 곳 |
|---|---|---|
| `/letter/` (권장) | 이번 주 호, 학년대는 가족이 고른다 | 그 기기 |
| `/letter/?t=<가족 토큰>` | 이번 주 호, 학년대는 **명부에서** 자동 | 서버(원장의 열람 현황에 잡힌다) |
| `/letter/?t=<잘못된 값>` | 이번 주 호, 체험 모드로 내려온다 | 그 기기 |

| 자리 | 내용 |
|------|------|
| `letter/issues/<id>.json` | 호 본문 (예: `2026-W41.json`). 파일 하나가 한 호 |
| `letter/issues.json` | 호 목록 — 파일명·id·주차·제목·주제·발행일·상태. 앱이 이것만 보고 오늘의 호를 고른다 |

**새 호를 내는 절차**: ① `letter/issues/<주차>.json` 을 만든다 ② `letter/issues.json` 에 한 줄 더한다(발행일은 그 주 월요일)
③ `node letter/issue-validate.mjs` 로 오류 0 확인 ④ PR → main. **발행일이 되면 앱이 저절로 그 호로 넘어간다** — 파일을 갈아 끼우러
다시 올 필요가 없다. 오늘 합쳐도 다음 주 호는 다음 주에 열린다. `status: "draft"` 인 호는 발행일이 지나도 가정에 나가지 않는다.

날짜 고르기는 `letter.js` 의 `pickPilot(list, today)` 하나다(순수 함수, `letter.test.cjs` 가 지킨다) — 앱과 테스트가 같은 규칙을 쓴다.
`/letter/*` 는 no-store 라 배포 즉시 반영된다. 배포본에는 `issue-pilot.json` 한 벌이 더 실리는데, 이는 **아직 새 껍데기를 못 받은 기기**(서비스 워커 갱신 전)를 위한
그날의 호 사본이다 — 저장소에는 두지 않고 `build-dist.mjs` 가 만든다. 목록을 못 읽으면 그 사본, 그것도 없으면 자체 창작 샘플 호(체험)로 넘어간다.
호 본문은 자체 창작만 저장소에 둘 수 있다(CLAUDE.md 절대 규칙 1). 정식 운영(가족 링크·서버 기록·푸시)으로 넘어갈 때는 같은 JSON 을 관리 웹 [편집]에 붙여 넣어 발행하면 된다.

**파일을 고쳤으면 합치기 전에 검증한다** — `node letter/issue-validate.mjs`(인자 없이 부르면 `issues/` 의 호 전부 + 샘플 + 목록 대조까지 본다).
규칙은 관리 웹 [검증]과 같은 `checkIssue` 하나다. `letter/issue-validate.test.mjs` 가 CI 에서도 같은 검사를 돌리므로, 오류가 있는 호·한 학년대가 빈 호·
목록에 안 적힌 호 파일은 PR 에서 막힌다 — 파일럿 호는 발행일이 되면 사람 손 없이 가정 화면에 뜨기 때문이다.
