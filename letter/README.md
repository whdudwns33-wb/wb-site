...
자체 창작 체험 호 `issue-sample.json`와 파일럿 호 `issues/*.json`을 함께 둔다. 정식 운영 호(issue)는 관리 웹에서 KV에 저장한다.
...
| 파일 | 무엇 |
|---|---|
| `index.html` | 학생·가족 앱. 모드 4개 — **가족 링크**(`/letter/?t=<ptoken>`, 로그인 없음, 기록은 기기와 서버) · **학생**(`wbr.auth` 토큰, 기록을 서버에 저장) · **관리 미리보기**(`?id=&tier=` + 관리 토큰, 초안도 봄, `tier=all`·`print=1`/`print=nokey`) · **체험·파일럿**(연동 없이 배포본 이번 주 호, 실패 시 `issue-sample.json`, 기록은 기기에만). 문제 풀기(즉시 해설)·정답 보기·미션 체크·[다 읽었어요]·지난 호·[PDF로 저장](정답 별지 포함 / 정답 없이). 가족 링크 화면에는 「오늘 5분 끊어읽기 — 청크브레인」 카드(`/chunk/?t=` 같은 토큰)가 호가 없는 주에도 뜬다 |
...
| `../reading-server/public/letter-admin.html` | 관리 — 호 목록(발행/내리기·알림 다시 보내기·미리보기·인쇄 링크·삭제, 다음 주 달력 주제) · 편집(JSON + 머리 정보 폼 + 검증 + 학년대별 미리보기 + 사진·도형 스니펫 도우미) · AI 초안(7조각, 달력 프리필, VSI 조각은 생성기 seed 로 채움) · 주제 달력(44주 표 편집) · 사진(브라우저에서 1600px JPEG 로 줄여 올리기, 목록·삭제, **AI 삽화 만들기** — Gemini 이미지 모델, 미리 보고 마음에 들면 올리기) · 발송 문구(가족 링크 자동 발급) · 열람 현황 · 학생 학년대 지정 |
...
section 공통: { id: [a-z0-9-]{2,30} 유일, type, tiers: "all" | ["K","E1",…], title(≤80), image?: 사진(미션 제외), images?: [사진×3](읽을거리·칼럼·게시판) }
  ...
  column    { byline?, paragraphs:[…], takeaway? }      coach { tips:[…] 1~6 }      notice { items:[…] 1~10 }      checklist { items:[…] 1~8, itemTiers?: ["all" | ["K","E1",…], …] }
...
`checklist.itemTiers`는 `items`와 길이가 같아야 한다. 생략하면 모든 항목을 보여 주며, 있으면 자기 학년대 항목만 표시·집계한다.
항목을 숨겨도 원래 `섹션 id:항목 번호`를 유지한다. 보기 분산도 화면에 표시하는 순서만 바꾸며 저장된 객관식 답 번호는 원본 선택지 번호다.
...
호 본문은 자체 창작만 저장소에 둘 수 있다(CLAUDE.md 절대 규칙 1). 정식 운영(관리 웹에서 호 발행·새 호 푸시)으로 넘어갈 때는 같은 JSON 을 관리 웹 [편집]에 붙여 넣어 발행하면 된다. 유효한 가족 링크의 서버 기록은 파일럿에서도 동작한다.
...
- AI 초안: `ANTHROPIC_API_KEY`(모델 기본 `claude-opus-5`, `LETTER_AI_MODEL` 로 변경), 하루 한도 `LETTER_AI_DAILY`(기본 30회, 한 호 7조각). 재시도와 AI 삽화도 같은 한도를 사용한다. 키가 없으면 초안 버튼만 안내로 동작하고 나머지는 그대로 돈다.
...
이 앱 테스트 전부 실행: `node scripts/check.mjs --only letter`
...
**아직 없는 것(다음 단계)**: 학생별 맞춤(진로독서 관심사·워드브레인 낱말과 연결, 검사 결과 기반 약한 지표 우선) · 지난 호 검색 · 이메일 발송(스티비) 연동 · 형제 가정 링크 하나로 묶기. 읽어 주기는 `shared/voice.js`로 이미 제공한다.
...
| 일째 | 장 | 무엇을 |
|---|---|---|
| 1 | 읽을거리 | 기본: 머리기사(표지·머리말) + 읽을거리 본문·낱말 노트(문제는 없다). 선택: **시 한 편**(낭독·필사) + **가족 대화 카드** |
| 2 | 읽고 답해요 | 문제만(본문은 `<details>` 로 접혀 있다) + **한 문장 쓰기**(요약·질문 만들기·기자 되기) |
| 3 | 한자 코너 / 낱말 복습 | 낱말 가족 + **손가락 따라쓰기** + **낱말 복습**(이번 주 글의 낱말 노트에서 만든 빈칸 문제). 한자 코너가 없는 학년대는 장 이름도 낱말 복습 |
| 4 | 두뇌 놀이터 | 그 주 지표의 놀이 |
...
1~6일째에는 **오늘의 5분**(`drills.js`)이 매일 맨 아래 같은 자리에 한 문제 놓인다. 일일 놀이는 작업기억·처리속도·유동추론·언어이해 네 지표를 다루고, 시공간을 포함한 다섯 지표는 주간 두뇌 놀이의 **5주 순환**으로 만난다. 요약·질문 만들기·어휘·필사·가족 토론은 읽은 내용을 말과 글로 표현하는 활동으로 배치했다.

그 학년대에 그날 섹션이 없으면 읽을거리를 **다시 읽는 날**로 채운다. 3일째는 낱말 복습을 별도로 제공한다. 잠그지는 않는다 — 7일 띠에서 어느 날이든 열 수 있고(지난 장 다시 열기가 곧 복습), 오늘 장은 진하게·마친 날은 ✓·아직 오지 않은 날은 옅게 보인다. [오늘 장 다 읽었어요]가 `state.days[n]` 에 남고 7일째 것은 호 완독(`doneAt`)이다. `?view=paper` 또는 [신문 전체로 보기]로 예전 신문 한 면도 본다. 인쇄(PDF)는 늘 신문 전체다. 앱 모드 옵션 `noQuiz`·`foldText`·`noTts`·`noTrace` 는 `renderDay` 가 장에 맞춰 넣는다(관리 미리보기는 `noTts`·`noTrace`).
