...
const SHELL = ['./', './index.html', './letter.js', './shapes.js', './manifest.webmanifest', './icon.svg'];
...
self.addEventListener('activate', (e) => {
  e.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((k) => k !== VERSION && k !== META).map((k) => caches.delete(k)))).then(() => self.clients.claim()));
});
