...
단원별로 시험 보고, 부족한 낱말을 배우고, 다시 확인한다. 한자는 낱말 이해를 돕는 선택 기능이며
글자만 있는 급수 교재는 별도의 한자 트랙이다. 영어는 다루지 않는다(영어는 워드브레인 `/vocab/`).
...
```
hanja/
  index.html        학생 앱 (주메뉴: 오늘 할 일·내 교재·복습·학습 기록 — 낱말·한자 학습은 교재와 단원에서 연다)
